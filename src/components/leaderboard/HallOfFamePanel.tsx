...
export async function HallOfFamePanel({
  standings,
  membershipId,
}: {
  standings: Standings;
  ...
  const [legends, badges, awards] = await Promise.all([
    getLegends(db),
    getBadgeCabinet(db, membershipId),
    listWeeklyAwards(db, standings.season.id, metricNames),
  ]);
  ...
  return (
    <section id="hall-of-fame" className="border-line scroll-mt-24 border-t pt-7">
      <div className="mb-5">
        <SectionTitle>HALL OF FAME</SectionTitle>
        <p className="text-ink-3 mt-1 text-[12px] font-semibold">
          Legends, badge cabinet and MVP roll alongside the live standings.
        </p>
      </div>
            ...
            {legends.length === 0 && (
              <Card>
                <p className="text-ink-2 text-[14px] font-semibold">
                  No completed weeks yet. Champions appear once the weekly
                  rollup has run.
                </p>
              </Card>
            )}
